...
namespace aspect
{
  ...
  namespace ShearBands
  {
     ...
     * An initial conditions model for the shear bands benchmark
     * that implements a background porosity plus white noise with
     * a certain amplitude and adds a melt pocket at the domain center.
     */
    template <int dim>
    class ShearBandsWithPocket : public InitialComposition::Interface<dim>,
      public ::aspect::SimulatorAccess<dim>
    {
      public:
    ...
    template <int dim>
    void
    ShearBandsWithPocket<dim>::initialize ()
    {
      AssertThrow(Plugins::plugin_type_matches<const ShearBandsMaterial<dim>>(this->get_material_model()),
                  ExcMessage("Initial condition shear bands only works with the material model shear bands."));
    ...
    template <int dim>
    double
    ShearBandsWithPocket<dim>::
    initial_composition (const Point<dim> &position, const unsigned int /*n_comp*/) const
    {
      // Calculate squared distance relative to radius
      double dist_sq = std::pow((position[0] - cx) / rad, 2);
      if (dim >= 2)
        dist_sq += std::pow((position[1] - cy) / rad, 2);

      double result;

      //If inside the ellipse (dist_sq <= 1), use Gaussian pocket.
      // Otherwise, use the noisy background.
      if (dist_sq <= 1.0)
        {
          // Gaussian: phi = phi0 + (phi_p - phi0) * exp(-dist_sq)
          result = background_porosity + (phi_pocket - background_porosity) * std::exp(-dist_sq);
        }
      else
        {
          result = background_porosity + interpolate_noise->value(position);
        }

      //Final clamping to stay within physical/numerical bounds
      return std::max(min_phi, std::min(max_phi, result));
    }


    template <int dim>
    void
    ShearBandsWithPocket<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Initial composition model");
      {
        prm.enter_subsection("Shear bands with pocket");
        {
          prm.declare_entry ("Noise amplitude", "0.0005",
                             Patterns::Double (0),
                             "Amplitude of the white noise added to the initial "
          ...
          prm.declare_entry ("Grid intervals for noise Y", "25",
                             ...
                             "to the model grid. "
                             "Units: none.");
          prm.declare_entry ("Pocket peak porosity", "0.05",
                            Patterns::Double (0), "Peak porosity at center.");
          prm.declare_entry ("Pocket center X", "2",
                            Patterns::Double (0), "X coordinate of pocket center (km).");
          prm.declare_entry ("Pocket center Y", "0.5",
                            Patterns::Double (0), "Y coordinate of pocket center (km).");
          prm.declare_entry ("Pocket radius", "0.3",
                            Patterns::Double (0), "Radius of the pocket (km).");
          prm.declare_entry ("Min porosity limit", "0.01",
                            Patterns::Double (0));
          prm.declare_entry ("Max porosity limit", "0.1",
                            Patterns::Double (0));
        }
        prm.leave_subsection();
      }
    ...
    template <int dim>
    void
    ShearBandsWithPocket<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Initial composition model");
      {
        prm.enter_subsection("Shear bands with pocket");
        {
          ...
          grid_intervals[0]    = prm.get_integer ("Grid intervals for noise X");
          grid_intervals[1]    = prm.get_integer ("Grid intervals for noise Y");
          phi_pocket           = prm.get_double ("Pocket peak porosity");
          cx                   = prm.get_double ("Pocket center X");
          cy                   = prm.get_double ("Pocket center Y");
          rad                  = prm.get_double ("Pocket radius");
          min_phi              = prm.get_double ("Min porosity limit");
          max_phi              = prm.get_double ("Max porosity limit");
        }
        prm.leave_subsection();
      }
      prm.leave_subsection();
...
namespace aspect
{
  namespace ShearBands
  {
    ...
    ASPECT_REGISTER_INITIAL_COMPOSITION_MODEL(ShearBandsWithPocket,
                                          "shear bands with pocket",
                                          "Composition is set to background porosity plus "
                                          "white noise and a central melt pocket.")
