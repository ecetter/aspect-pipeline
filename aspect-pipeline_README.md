...
This repository provides a container-based workflow for the [ASPECT software](https://aspect.geodynamics.org), enabling source code customization, large-scale parallel builds, and reproducible execution on HPC systems using Slurm, specifically [ICDS@PSU](https://icds.psu.edu)'s [Roar Collab cluster](https://icds.psu.edu/services/roar/roar-collab-cluster). The workflow is built around Apptainer/Singularity containers and is designed to combine developer flexibility with portability, performance, and reproducibility.
...
1. **Download and Modify ASPECT Source Code**
   Users download ASPECT source code automatically using the provide download script.
   ...
   Multiple source code variants may be saved at any time, and the "aspect" soft link dictates the source code to build.
...
4. **Test and Benchmark**  
   The `test/` directory integrates ASPECT cookbooks.  
...
The host system must utilize Slurm as the job scheduler for build and test scripts.
...
1. Download and modify ASPECT source code in `src/`
2. Submit container build job from `build/`
3. Set up and run simulations in `run/`
4. Execute automated benchmarks from `test/`
